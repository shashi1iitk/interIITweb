...
def addMatches():
    ...
    if (request.method == 'POST'):
        if (request.form.get ( 'type' ) == "team"):
            ...
            entry = Match_Individual ( sport_id = request.form.get ( 'sport' ) ,
                                       clgs_playing = colleges , players = players ,
                                       date_time = request.form.get ( 'datetime' ) ,
                                       venue = request.form.get ( 'venue' ) , clg_1st = 0 , clg_2nd = 0 , clg_3rd = 0 ,
                                       clg_4th = 0 ,
                                       clg_1st_player_id = 0 , clg_2nd_player_id = 0 , clg_3rd_player_id = 0 ,
                                       clg_4th_player_id = 0 ,
    ...
    players = [{"id": x.id , "name": x.name ,
                "clg_name": (College.query.filter ( College.id == int ( x.college_id ) ).first ( )).clg_name} for x in
               players
